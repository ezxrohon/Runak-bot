...
from .common import to_small_caps
...
async def _handle_ai_reply(client: Client, message: Message):
    ...
    try:
        ...
        reply = to_small_caps("Arre ek sec busy thi, phir se bolo na") + " 🫧"
    except Exception:
        log.warning("AI reply failed", exc_info=True)
        return  # fail silently rather than spam the chat with an error

    if reply:
        # Every chat-bot reply goes out in small caps (ᴀʙᴄ style, same look
        # used for BOT_NAME / admin titles elsewhere in this bot).
        await message.reply_text(to_small_caps(reply))
