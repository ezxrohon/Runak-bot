...
def register_economy_handlers(app: Client):
    ...
    async def leaderboard_cmd(client, message: Message):
        ...
        lines = [f"🏆 {CURRENCY_NAME} Leaderboard (Global)"]
    ...
    async def group_leaderboard_cmd(client, message: Message):
        ...
        lines = [f"🏆 {CURRENCY_NAME} Leaderboard — {message.chat.title}"]
        for i, entry in enumerate(top, start=1):
            lines.append(f"{i}. {entry.get('first_name', 'Unknown')} — {entry.get('balance', 0)} {CURRENCY_EMOJI}")
        await message.reply_text("\n".join(lines))
    ...
    async def profile_cmd(client, message: Message):
        ...
        await message.reply_text(
            f"👤 {target.first_name}'ꜱ Pʀᴏꜰɪʟᴇ\n\n"
            f"☠️ Dᴀɪʟʏ Kɪʟʟꜱ : {daily_kills}/{DAILY_KILL_LIMIT}\n"
            f"🔪 Dᴀɪʟʏ Rᴏʙꜱ : {daily_robs}/{DAILY_ROB_LIMIT}\n"
            f"💼 Wᴀʟʟᴇᴛ : {wallet}"
        )
