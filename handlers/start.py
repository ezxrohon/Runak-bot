...
"""
        ...
        if MUSIC_ENABLED:
            rows.append([InlineKeyboardButton("🎵 Music ⌂", callback_data="help_music")])
...
"""
    ...
    async def send_games_menu(message):
        text = """
🎮 Gᴀᴍᴇs
─────────────────────────────
...
"""
    ...
    async def help_games_menu_callback(client, callback_query):
        await send_games_menu(callback_query.message)
        await callback_query.answer()
...
/playforce <name|link> — skip current and play now (admin)
/vplayforce <name|link> — skip current and video-play now (admin)
/queue or /q — show what's playing and up next
/skip — play the next track in queue (admin)
/pause / /resume — pause or resume playback (admin)
/seek <secs> / /seekback <secs> — jump forward/back (admin)
/restart — replay the current track from 0:00 (admin)
/loop <0-10> — repeat the current track (admin)
/stop or /end — stop and clear the queue (admin)

Anyone can /play; the rest are admin-only so randoms can't hijack
the queue. (Owner: requires SESSION_STRING set in .env — see
config.py. If unset, this feature is silently disabled.)
""",
        "help_owner": """
👑 Owner Tools
─────────────────────────────
...
""",
    ...
    async def help_section_callback(client, callback_query):
        ...
        back_target = "help_games_menu" if callback_query.data in GAMES_SUBPAGES else "help"
